...
// One-off: bump the exchange-rate figure in every tour's paymentDetails.
// Handles both 23,000 and 23.000 groupings; updates en + vi.
const FROM = ['23,000', '23.000', '23000'];
const TO = ['26,000', '26.000', '26000'];

function replaceRate(text: string): string {
  let out = text;
  FROM.forEach((from, i) => {
    out = out.split(from).join(TO[i]);
  });
  return out;
}
